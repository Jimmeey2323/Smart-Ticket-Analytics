...
export const DynamicForm: React.FC<DynamicFormProps> = ({
  ...
  const sectioned = useMemo(() => {
    ...
    return [...sections, ...formSections];
  }, [visibleFields, shouldShowClientSection, fieldGroups, context]);
  ...
  const renderField = (field: FieldDefinition) => {
    ...
    const renderLabel = () => (
      <Label htmlFor={field.id} className={`text-sm font-semibold ${disabled ? 'text-muted-foreground' : 'text-foreground'}`}>
        {field.label}
    ...
    switch (field.fieldType) {
      ...
      case 'Phone':
        return (
          <div key={field.id} className={`space-y-2 rounded-xl border-2 border-border bg-muted p-4 shadow-sm hover:shadow-md transition-all ${getFieldSpanClass(field)}`}>
            {renderLabel()}
            <Input
              {...fieldProps}
              ...
              className={`bg-input border-2 ${error ? 'border-red-500' : 'border-input'} focus:border-blue-600 focus:ring-2 focus:ring-blue-100 text-foreground shadow-sm`}
              disabled={disabled}
            />
            {field.description && (
        ...
        return (
          <div key={field.id} className={`space-y-2 rounded-xl border-2 border-border bg-muted p-4 shadow-sm hover:shadow-md transition-all ${getFieldSpanClass(field)}`}>
            ...
            <Select
              ...
              disabled={disabled}
            >
              <SelectTrigger className={`bg-input border-2 ${error ? 'border-red-500' : 'border-input'} focus:border-blue-600 focus:ring-2 focus:ring-blue-100 text-foreground shadow-sm`}>
                <SelectValue placeholder={`Select ${field.label}`} />
              </SelectTrigger>
              <SelectContent className="bg-input border-2 border-input shadow-lg">
  ...
  return (
    <div className="w-full">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {sectioned.length > 1 && (
          <div className="bg-card rounded-xl border-2 border-card-border shadow-lg p-4 mb-6">
            ...
            {(() => {
              ...
              return (
                <>
                        ...
                        <div className="rounded-xl border-2 border-blue-200 bg-blue-50 p-6 text-center">
                          <p className="text-gray-700 mb-4">
                            {context?.categoryName && context?.subCategoryName ? (
                              <>
                                <span className="font-semibold text-foreground">{context.categoryName}</span>
                                {' → '}
                                <span className="font-semibold text-foreground">{context.subCategoryName}</span>
                              </>
                            ) : (
                  ...
                  {/* Navigation Buttons */}
                  <div className="flex items-center justify-between pt-8 mt-8 border-t border-input">
                    <Button
                      type="button"
                      variant="outline"
